...
// minimal feather-style icon set (mirrors design's chrome.jsx)
export const icons: Record<string, string> = {
  ...
  mapPin: "M12 22s-8-7.5-8-13a8 8 0 1 1 16 0c0 5.5-8 13-8 13zM12 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6z",
};
...
export function Icon({ name, size = 14, className, style, strokeWidth }: IconProps) {
  const d = icons[name];
  if (!d) return null;
  return (
    <svg
      className={`${size === 16 ? "ico ico-lg" : "ico"}${className ? " " + className : ""}`}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      style={style}
